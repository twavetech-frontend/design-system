 ...
 */
export function transformFontWeight(token) {
    const val = token.$value ?? token.value;
    const type = token.$type ?? token.type;
    if (val === undefined)
        return undefined;
    const transformWeight = (fontWeight) => {
        ...
        const weight = match.groups.weight.toLowerCase();
        ...
        if (!style && !fontWeightMap[weight] && fontStyles.includes(weight)) {
            ...
            mapped = fontWeightMap[weight.replace(/\s/g, '')] ?? weight;
        }
        if (weight && style) {
            mapped = `${mapped} ${style}`;
        }
